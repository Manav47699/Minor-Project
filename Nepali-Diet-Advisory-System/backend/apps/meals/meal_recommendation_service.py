...
def assemble_recommendation_payload(meal: MealLog) -> dict:
        ...
        food_items_payload.append(
            {
                ...
                "micronutrients": getattr(item, "micronutrients", {}),
                "veg_or_nonveg": "",
                "fitness_direction": "",
                "health_warnings": [],
            }
    ...
    historical_foods = []
    try:
        from .models import MealFoodItems
        historical_foods = list(
            MealFoodItems.objects.filter(food_analysis__meal__user=user)
            .exclude(food_analysis__meal=meal)
            .values_list("food_name", flat=True)
            .distinct()[:30]
        )
    except Exception as err:
        logger.warning(f"Could not retrieve historical food logs for user {user.id}: {err}")
    ...
    payload = {
        ...
        "nutrition_summary": {
            ...
            "micronutrients": getattr(analysis, "micronutrients", {}),
        },
        "food_items": food_items_payload,
        "user_profile": user_profile_payload,
        "historical_foods": historical_foods,
    }
